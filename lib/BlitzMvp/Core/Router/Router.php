...
class Router {
	 ...
	 * @return string
	 */
	private function _cleanPath($path, $root = BMVP_PRESENTERS) {
	...
	public function getViewRoute($template = '') {
		$templatePath = BMVP_VIEWS . "$template.twig";
		if(!is_file(BMVP_ROOT . $templatePath))
			$templatePath = BMVP_VIEWS . self::DefaultTemplate . ".twig";
		if(!is_file(BMVP_ROOT . $templatePath)) {
